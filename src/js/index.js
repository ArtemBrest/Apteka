    ...
    */
    ...
    if(feedbackForm !== null) {
        ...
        feedbackForm.addEventListener('submit', (event) => {
            ...
            if (!regexSubject.test(feedbackFormName.value) || !mail.test(feedbackFormEmail.value) || !regexPhone.test(feedbackFormPhone.value) || !regexSubject.test(feedbackFormCompanyValue)) { //|| radioValue == "Выберете тему обращения"
                if (!regexSubject.test(feedbackFormName.value)) {
                    feedbackFormName.parentNode.classList.add("error");
                }
                if(feedbackFormCompany !== null) {
    ...
    if(!isEmptyObject(accountPasswordInput)){
        accountPasswordInput.forEach(function (input) {
            let parent = input.parentNode;
            let passwordBtnClose = parent.querySelector(".password-btn-close");
            let passwordBtnOpen = parent.querySelector(".password-btn-open");
            passwordBtnClose.addEventListener("click", function (e){
                passwordBtnClose.classList.add("hidden");
                passwordBtnOpen.classList.remove("hidden");
            ...
            document.querySelector(".account-password__btn").addEventListener("click",function (e) {
                if(document.getElementById("new-password").value.length < 1){
                    return;
                }
                else if(document.getElementById("new-password").value == document.getElementById("confirm-password").value){
                    document.getElementById("confirm-password").parentNode.classList.remove("error");
                    document.getElementById("confirm-password").parentNode.classList.add("valid");
                }
                else{
                    document.getElementById("confirm-password").parentNode.classList.remove("valid");
